...
from src.utils.io import JsonlWriter, failure_writer, read_jsonl
...
def run_phrase_worker(adapter: PhraseGrounder, args: argparse.Namespace) -> None:
    ...
    progress = JsonlWriter(progress_path)
    ...
    for index, raw_request in enumerate(requests):
        ...
        if request.request_id in done:
            continue
        print(f"[ground_{adapter.name}] request={index + 1}/{len(requests)} "
              f"image={request.image_id} START", flush=True)
        started = time.perf_counter()
        image = None
        try:
            detections = adapter.ground(request.image_path, request.phrase)
            if args.save_crops or args.save_overlays:
                image = open_rgb(request.image_path)
            saved_count = 0
            for detection_index, detection in enumerate(detections):
                box = clip_box(detection["bbox_xyxy"], request.image_width, request.image_height)
                if not valid_box(box, request.image_width, request.image_height):
                    continue
                request_tag = hashlib.sha1(request.request_id.encode("utf-8")).hexdigest()[:10]
                stem = (
                    f"{request.image_id}_{request.rank:02d}_{request_tag}_"
                    f"{adapter.name}_{detection_index:02d}"
                )
                crop_path: Path | None = None
                overlay_path: Path | None = None
                mask_path: Path | None = None
                if args.save_crops:
                    crop_path = artifact_root / "crops" / request.route / f"{stem}.jpg"
                    save_crop(image, box, crop_path)
                if args.save_overlays:
                    overlay_path = artifact_root / "overlays" / request.route / f"{stem}.jpg"
                    save_overlay(image, box, request.phrase, overlay_path)
                if args.save_masks and detection.get("mask") is not None:
                    mask_path = artifact_root / "masks" / request.route / f"{stem}.png"
                    save_mask(np.asarray(detection["mask"]), mask_path)
                metadata = {
                    **request.metadata,
                    **adapter.provenance,
                ...
                result = RegionRecord(
                    ...
                    grounder=adapter.name,
                    grounding_score=detection.get("score"),
                    image_width=request.image_width,
                    image_height=request.image_height,
                    runtime_ms=(time.perf_counter() - started) * 1000,
                    metadata=metadata,
                )
                writer.append(result.model_dump(mode="json"))
                saved_count += 1
            progress.append(
                {
                    "request_id": request.request_id,
                    "image_id": request.image_id,
                    "detections": saved_count,
                    "context_signature": getattr(args, "context_signature", ""),
                }
            )
            ...
            failure_writer(
                ...
                image_id=request.image_id,
                route=request.route,
                stage=f"ground_{adapter.name}",
                error_type=type(exc).__name__,
                message=str(exc),
                request_id=request.request_id,
            )
            raise RuntimeError(
                f"ground_{adapter.name}: fatal request {request.request_id}: {exc}"
            ) from exc
        finally:
            print(f"[ground_{adapter.name}] request={index + 1}/{len(requests)} "
                  f"saved={request.request_id in done} elapsed={time.perf_counter() - started:.1f}s",
                  flush=True)
